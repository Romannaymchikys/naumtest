...
int CalculationStep(bool value,  int& step,
	...
	++j; ++i; sum += i;
	// если сумма четная то двигаемся в перед
	if (value) {
		// если i четное то  двигоемся по горизонту
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT uMessage, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_LBUTTONDOWN:
		SetClassLong(hWnd, GCL_HCURSOR,
			        (LONG)LoadCursor(NULL, IDC_ARROW));// меняем курсор на стандартный
		break;
	case WM_CLOSE:
